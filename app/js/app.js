  ...
  function wireLeagueToggle(id) {
    ...
    el.addEventListener('change', function () {
      if (this.checked) {
        ...
        var offNow = function () {
          Cloud.setEnabled(false);
          Cloud.forget().then(function () {
            toast('전송을 끄고 서버의 리그 기록을 삭제했습니다.');
          }, function () {
            toast('전송을 껐습니다. 서버 삭제는 연결되는 즉시 다시 시도합니다.');
          });
          League.setCloudRows([], '');
          renderCloudSettings();
          renderLeague();
  ...
  function initStudentShare() {
    ...
    $('studentShareOn').addEventListener('change', function () {
      if (this.checked) {
        ...
        Cloud.setStudentShareEnabled(false);
        Cloud.forgetStudent().then(function () {
          toast('공유를 끄고 서버의 관리자 공개 기록을 삭제했습니다.');
        }, function () {
          toast('공유를 껐습니다. 서버 삭제는 연결되는 즉시 다시 시도합니다.');
        });
        renderStudentShareSettings();
      }
    });
