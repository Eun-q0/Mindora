...
-- 로그인 없는 서비스라 auth.uid() 로 본인을 확인할 수 없다. 대신 앱이 만든
-- 무작위 UUID 를 삭제 권한표처럼 사용한다. UUID 를 아는 기기만 자기 행을
-- 지울 수 있고, 원본 테이블은 여전히 RLS 로 직접 읽거나 수정할 수 없다.
-- 두 공유 스위치는 서로 독립적이므로 삭제 함수도 범위를 나눈다.
