...
(function () {
  ...
  /* 칸 채우기용 파스텔 — 같은 과목끼리 같은 색이 되도록 자동 배정한다.
   * 인스타에 올라오는 계획표들이 과목마다 색을 달리 칠하는 걸 따라간 것. */
  var VP_CELL_COLORS = [
    '#ffe3e3', '#ffefd6', '#fdf8d3', '#e2f6da', '#d9f1f0',
    '#dde9fb', '#e7dffb', '#fbdfee', '#eeeeee', '#e6ded5'
  ];
  ...
  /** 평일 5칸 + 주말 2칸을 한 줄로 만든다 (주말을 안 주면 7일 모두 같은 값) */
  function vpRow(time, weekday, weekend) {
    var wknd = weekend === undefined ? weekday : weekend;
    return { time: time, cells: [weekday, weekday, weekday, weekday, weekday, wknd, wknd] };
  }
  ...
  function vpDefaultModel() {
    return {
      ...
      rows: [
        ...
        vpRow('13:00', '영어', '자유시간'),
        vpRow('14:00', '영어', '자유시간'),
        vpRow('15:00', '자율 / 보충', '운동'),
        vpRow('16:00', '자율 / 보충', '운동'),
        vpRow('17:00', '휴식'),
        vpRow('18:00', '저녁식사'),
        vpRow('19:00', '자기주도학습'),
        vpRow('20:00', '자기주도학습'),
  ...
  /** 새로 등장한 과목에 파스텔 색을 차례로 배정한다 (이미 정해진 색은 건드리지 않는다) */
  function vpEnsureColors(m) {
    ...
    m.rows.forEach(function (r) {
      r.cells.forEach(function (c) {
        ...
        m.colors[t] = VP_CELL_COLORS[idx % VP_CELL_COLORS.length];
        idx++;
      });
    });
  }
  ...
  function renderVpLegend() {
    ...
    wrap.innerHTML = list.map(function (t) {
      return '<label class="vp-lg"><input type="color" value="' + esc(m.colors[t]) + '">' +
        '<span>' + esc(t) + '</span></label>';
    }).join('');

    // 과목 이름을 속성에 넣으면 따옴표·꺾쇠 때문에 깨질 수 있어 순서(index)로 잇는다
    $$('input[type="color"]', wrap).forEach(function (inp, i) {
      inp.addEventListener('input', function () {
        m.colors[list[i]] = inp.value;
        vpSave();
        buildVpTable();
        vpRenderPreview();
      });
